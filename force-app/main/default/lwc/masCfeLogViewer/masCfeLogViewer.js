...
import getLogs from '@salesforce/apex/MAS_CFE_ConfigController.getLogs';
...
export default class MasCfeLogViewer extends LightningElement {
    /** MAS_CFE_Configuration__c record Id. */
    @api recordId;
    ...
    // new MAS_CFE_Log__c rows land, so a manual refresh is the only way to see them
    ...
    wiredLogs(result) {
        ...
        if (data) {
            this.logs = data.map((row) => ({
                ...row,
                logUrl: `/lightning/r/MAS_CFE_Log__c/${row.Id}/view`,
