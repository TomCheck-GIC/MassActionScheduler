...
import getLogs from '@salesforce/apex/MAS_GIC_ConfigController.getLogs';
...
export default class MasGicLogViewer extends LightningElement {
    /** MAS_GIC_Configuration__c record Id. */
    @api recordId;
    ...
    // new MAS_GIC_Log__c rows land, so a manual refresh is the only way to see them
    ...
    wiredLogs(result) {
        ...
        if (data) {
            this.logs = data.map((row) => ({
                ...row,
                logUrl: `/lightning/r/MAS_GIC_Log__c/${row.Id}/view`,
